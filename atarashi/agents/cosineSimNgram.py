...
@plac.annotations( 
  processedLicense = plac.Annotation("Specify the processed license list file", "optional", str, metavar="PROCESSEDLICENSELIST"),
  ngramJsonLoc = plac.Annotation("Specify the location of NGRAM JSON", "positional", metavar="NGRAM_JSON"),
  inputFile = plac.Annotation("Specify the input file which needs to be scanned", "positional"),  
  similarity = plac.Annotation("Specify the similarity algorithm that you want", "positional", "s", str, ["CosineSim", "DiceSim", "BigramCosineSim"]),  
  verbose = plac.Annotation("Increase output verbosity", "flag", "v")
)


def main(processedLicense, ngramJsonLoc, inputFile, similarity="BigramCosineSim", verbose=0):
